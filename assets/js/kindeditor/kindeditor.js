...
KE.setting = {
	...
	items : [
		...
		'italic', 'underline', 'strikethrough', 'removeformat', '|', 'upload', 'image',
		'flash', 'media', 'advtable', 'hr', 'excerpt', 'emoticons', 'link', 'unlink', '|', 'about'
	],
	colorTable : [
...
KE.create = function(id, mode) {
	...
	if (!KE.g[id].wyswygMode) {
		...
		iframe.style.display = 'none';
		KE.toolbar.disable(id, ['source', 'fullscreen', 'theme']);
...
plugins.upload = {
	ulFile : '选择文件',
	ulExp : '本系统仅支持 doc/xls/ppt/pdf/txt/rar/zip 类型的附件上传。'
};
})(KindEditor);
...
KE.plugin['upload'] = {
	...
	insert : function(id, url,fileName) {
		var html = '附件：<a href="' + url + '">'+ fileName +'</a>';
...
KE.plugin['code'] = {
	...
	exec : function(id){
		var dialogDoc = KE.util.getIframeDoc(this.dialog.iframe);
		var dialogLang = KE.$('lang', dialogDoc).value;
		var dialogCode = KE.$('code', dialogDoc).value;
		if (!this.check(id)) {
			return false
		}
		KE.util.insertHtml(id, '<pre class="brush:' + dialogLang + ';">' + this.escape(dialogCode) + "</pre>");
		this.dialog.hide();
		KE.util.focus(id);
	}
};
